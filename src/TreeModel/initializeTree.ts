...
function fitToScreen(leveledNodes: [TreeNode, number][]) {
  const startX = 0;
  const startY = 0;
...
const C = new TreeNode(
  'C',
  [
    new TreeNode('daughter1', []),
    new TreeNode('daughter2', []),
    new TreeNode('daughter3', []),
  ],
  new TreeNode('c spouse', []),
);
